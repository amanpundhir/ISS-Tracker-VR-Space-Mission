        ...
        if(allDebris.length==0){
            for(let i = 0; i < json.length; i++){
                if(json[i].OBJECT_TYPE=='DEBRIS'){
                    ...
                    var pos = get_iss_pos(satrec, time); // Assuming get_iss_pos is defined elsewhere
...
var time = get_render_time(); // Assuming get_render_time is defined elsewhere
...
function updateDebris(){
    ...
    for(let i = 0; i < allDebris.length; i++){
        try {
            var pos = get_iss_pos(allSatrec[i], time); // Assuming get_iss_pos is defined elsewhere
            allDebris[i].position['latitude'] = pos[0];
            allDebris[i].position['longitude'] = pos[1];
            allDebris[i].position['altitude'] = pos[2];
            var distance = getDistance(colladaLoader.position, allDebris[i].position); // Assuming colladaLoader.position is defined elsewhere
    ...
    if(prevMinIdx != -1){
        ...
        var pos = get_iss_pos(allSatrec[prevMinIdx], time); // Assuming get_iss_pos is defined elsewhere
        var debris = new WorldWind.Placemark(
            new WorldWind.Position(pos[0], pos[1], pos[2])
        );
        debrisLayer.addRenderable(debris);
...
function getDistance(pos1, pos2){
    var carPos1 = sph2car(pos1['latitude'], pos1['longitude'], pos1['altitude'] / 1000); // Assuming sph2car is defined elsewhere
    var carPos2 = sph2car(pos2['latitude'], pos2['longitude'], pos2['altitude'] / 1000); // Assuming sph2car is defined elsewhere
